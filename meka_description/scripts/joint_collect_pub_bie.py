...
def shm_humanoid(joint_states):
	...
	elif joints[i] == 'torso_j2':
	    for k in range(len(joint_states.name)):
	      if joint_states.name[k] == 'torso_j1':
		positions[i] = joint_states.position[k]
		velocities[i] = joint_states.velocity[k]
		effort[i] = joint_states.effort[k]
...
positions.append(0.0*3.14/180.0)
velocities.append(0.0)
effort.append(0.0)
